    ...
    return (
        <div className={`m-[5px_0] transition-all relative ${state ? "p-[0_5%]" : "p-[4px]"}`} >
            <div className='flex items-center w-full'>
                <NavLink to={to} className={({ isActive }) => `flex items-center justify-between no-underline p-[calc(5px-2px)_0] h-[60px] w-full text-white  ${isActive ? "font-bold content-['']  bg-[#40690B] rounded-lg left-0" : ""} `} key={llave}>
            ...
            {subitems && isOpen && state && (
                <div className='mt-2'>
                    {subitems.map(({ icon: Icono, label, to, index }) => (
                        <NavLink key={index} to={to} className={({ isActive }) => `flex items-center justify-end no-underline py-2 px-2   text-white ${isActive ? "font-bold content-['']  bg-[#1c9c01] rounded-lg left-0" : ""} `}>
